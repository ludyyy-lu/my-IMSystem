...
### 用户在线状态维护
我们将“用户状态”视为一种临时、实时信息，不写入数据库，所以它不应该存在 User 表中。
**解决方案：**
用 Redis 存储用户状态（在线/离线、连接数等）
将状态维护逻辑放在 ws-gateway 模块中
提供接口由 user-service 暴露出来（对内 RPC 即可）

**目标功能：**
用户上线时标记为“在线”
用户离线后标记为“离线”
提供接口让别的服务能查询某个用户是否在线

**我们将：**
在 Redis 中维护用户在线状态（基于 Set）
实现 user-service 中的 IsOnline 接口，用于查询在线状态
在 ws-gateway 的连接管理逻辑中写入上线/离线逻辑（后面做）

Redis 中使用一个 Set 类型来维护所有在线用户的 UID：
```
Key: im:online:users
Type: Set
Value: UID（int64）
```
某用户上线时： SADD im:online:users <uid>
某用户下线时： SREM im:online:users <uid>
查询是否在线： SISMEMBER im:online:users <uid>
...
### gRPC拦截器（Interceptor）：实现统一 token 鉴权
目标： 在所有需要认证的 RPC 上，自动验证 token，提取用户信息，避免每个接口都手动校验。
---
...
### 25/08/05
ws-gateway 模块远程 token 鉴权逻辑，接入 auth-service 的 VerifyToken 接口。


### 25/08/06
将用户的在线状态 记录到 Redis 中（上线），并在下线时从 Redis 删除。
